def keras_lps_leftstitchfun(lpsdata,scandata):
    ...
    import sys
    import time
    from plotting_lps_fcns import plot_lps_vs_prediction_lucretia, grab_ns_plot_lps_vs_prediction_lucretia
...
# Reshape predictions on the test set
    ...
    ns = np.zeros((5,1))
    for i in range(5):
        ns[i] = grab_ns_plot_lps_vs_prediction_lucretia(lpstest,predict_lpstestreshaped,x,y)  
...
# Make a histogram of the score
    ...
    plt.show()
    print(np.mean(score))
    return predict_lps_test,lpstest,ns