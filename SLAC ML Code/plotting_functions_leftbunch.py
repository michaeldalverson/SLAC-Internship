...
def preset_plot_left_prediction_vs_lucretia(curprof,predicted_curprof,Imax,ns):
    import matplotlib.pyplot as plt
    import numpy as np
    for i in range(len(ns)):
        ...
        #ns = int(curprof[:,0].shape+curprof[:,0].shape*(0.5*(2*np.random.rand(1,1))-1))
        plt.figure()
        plt.plot(tvector,curprof[ns[i],:]*Imax,label='Simulation');
        plt.plot(tvector,predicted_curprof[ns[i],:]* Imax,'r--',label='Neural Net');
        plt.xlabel('z [um]')
        plt.ylabel('I [kA]')
        plt.legend()
        plt.show()